...
class _RankingsScreenState extends State<RankingsScreen> {
  ...
  final _boardSnapshots = <String, (List<RankingItem>, int, bool, String)>{};
  bool _loading = true;
  bool _more = false;
  bool _hasMore = false;
  ...
  Future<void> _load({bool more = false, bool force = false}) async {
    ...
    try {
      ...
      _boardSnapshots[board.id] = (_items, _page, _hasMore, _updated);
    } catch (error) {
      if (mounted && generation == _generation) {
        setState(() {
          _error = error.toString();
  ...
  void _select(RankingBoard board) {
    ...
    _generation++;
    if (_board != null && _items.isNotEmpty) {
      _boardSnapshots[_board!.id] = (_items, _page, _hasMore, _updated);
    }
    final snapshot = _boardSnapshots[board.id];
    setState(() {
      _board = board;
      _items = snapshot?.$1 ?? [];
      _updated = snapshot?.$4 ?? '';
      _page = snapshot?.$2 ?? 1;
      _hasMore = snapshot?.$3 ?? false;
      _loading = false;
      _more = false;
      _failedMore = false;
      _error = null;
