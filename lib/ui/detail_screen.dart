...
import '../downloads/download_picker.dart';
...
import '../playback/episode_browser.dart';
import '../library/follow_state.dart';
import '../library/hongguo_series.dart';
...
class _DetailScreenState extends State<DetailScreen> {
  static final _detailCache = Expando<Map<String, (DramaDetail, DateTime)>>();
  DramaDetail? _detail;
  String? _error;
  bool _loading = true;
  int _generation = 0;
  ...
  int? _inlineIndex;
  double _inlinePosition = 0;
  int _inlinePlayRequest = 0;
  final _episodeAnchor = GlobalKey();
  final _detailScroll = ScrollController();
  bool _initialActionHandled = false;
  late final int _profileEpoch;
  String get _cacheKey =>
      '${_profileEpoch}:${widget.drama.source}:${widget.drama.id}';

  void _rememberDetail(DramaDetail detail) {
    final entries =
        _detailCache[widget.repository] ?? <String, (DramaDetail, DateTime)>{};
    entries[_cacheKey] = (detail, DateTime.now());
    if (entries.length > 48) entries.remove(entries.keys.first);
    _detailCache[widget.repository] = entries;
  }

  void _handleInitialAction(DramaDetail detail) {
    if (!mounted || _initialActionHandled || detail.episodes.isEmpty) return;
    ...
    if (widget.resumeOnOpen) {
      unawaited(
        _play(
          resumeEpisodeIndex(
            detail.episodes,
            widget.store.watched(detail.drama.id),
          ),
          resume: true,
        ),
      );
  ...
  void initState() {
    ...
    final cached = _detailCache[widget.repository]?[_cacheKey];
    if (cached != null &&
      ...
      _episodePage =
          resumeEpisodeIndex(
            cached.$1.episodes,
            widget.store.watched(cached.$1.drama.id),
          ) ~/
          episodePageSize;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _handleInitialAction(cached.$1);
      });
    } else {
      _load();
    }
  ...
  Widget build(BuildContext context) {
    ...
    return CallbackShortcuts(
      ...
      child: phone
          ? _inlineIndex != null && _detail != null && allowed
                ? PlayerScreen(
                    detail: _detail!,
                    initialIndex: _inlineIndex!,
                    initialPosition: _inlinePosition,
                    playRequest: _inlinePlayRequest,
                    repository: widget.repository,
                    store: widget.store,
                    mobileDetails: _mobileScaffold(
                      drama,
                      episodes,
                      watched?.episode,
                      resumeIndex,
                    ...
                    onInlineBack: () => setState(() => _inlineIndex = null),
                  )
                : _mobileScaffold(
                    drama,
                    episodes,
  ...
  Widget _mobileScaffold(
    ...
    final start = _episodePage * episodePageSize;
    final visible = episodes.skip(start).take(episodePageSize).toList();
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          if (!allowed)
            const Center(
              child: StatusPanel(title: '当前用户无权访问', message: '请返回剧库后重新选择。'),
            )
          else
            CustomScrollView(
              controller: _detailScroll,
              slivers: [
                      ...
                      child: Stack(
                        ...
                        children: [
                          ...
                          Center(
                            child: FilledButton.icon(
                              key: const ValueKey('start-play'),
                              onPressed: _loading || episodes.isEmpty
                                  ? null
                                  : () => _play(resumeIndex, resume: true),
                              icon: const Icon(Icons.play_arrow_rounded),
                              label: Text(
                                currentEpisode == null ? '立即播放' : '继续播放',
                              ),
                            ),
                          ),
                        ],
                ...
                SliverToBoxAdapter(
                  child: Padding(
                    ...
                    child: _mobileInfo(drama, episodes.length),
                  ),
                ),
                SliverToBoxAdapter(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Material(
                          color: colors.primaryContainer.withValues(alpha: .8),
                ...
                if (_loading || _error != null || episodes.isEmpty)
                  SliverToBoxAdapter(
                    child: SizedBox(height: 190, child: _episodeStatus()),
                  )
                else ...[
                  SliverToBoxAdapter(
                    child: _episodeSummary(
  ...
  Widget _mobileInfo(Drama drama, int episodeCount) {
    final colors = Theme.of(context).colorScheme;
    final following = widget.store.following(drama.id) != null;
    final meta = [
      if (drama.onlineDate.length >= 4) drama.onlineDate.substring(0, 4),
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          ...
          children: [
            _mobileAction(
              following ? Icons.star_rounded : Icons.star_outline_rounded,
              following ? '已收藏' : '收藏',
              () => saveUserChange(
                context,
            ...
            if (widget.store.canDownload && widget.repository.supportsDownloads)
              _mobileAction(Icons.download_rounded, '下载', _download),
            _mobileAction(Icons.copy_rounded, '复制剧名', () {
              unawaited(Clipboard.setData(ClipboardData(text: drama.title)));
              ScaffoldMessenger.of(
                context,
              ).showSnackBar(const SnackBar(content: Text('已复制剧名')));
            }),
          ],
        ),
        ...
        if (_detail?.warning.isNotEmpty == true)
          Text(_detail!.warning, style: TextStyle(color: colors.error)),
      ],
    );
  }

  Widget _mobileAction(IconData icon, String label, VoidCallback onPressed) {
    final colors = Theme.of(context).colorScheme;
    return InkWell(
      onTap: onPressed,
      borderRadius: BorderRadius.circular(14),
