...
import 'chewie_platform_player.dart';
...
class _PlayerScreenState extends State<PlayerScreen>
  ...
  ChewiePlatformPlayer? _chewieBackend;
  ...
  void initState() {
    ...
    final layout = context
        .getElementForInheritedWidgetOfExactType<AppLayout>()
        ?.widget;
    if (widget.playerFactory == null &&
        widget.videoBuilder == null &&
        (Platform.isAndroid || Platform.isIOS) &&
        (layout is! AppLayout || !layout.television)) {
      _chewieBackend = ChewiePlatformPlayer();
    }
    _player =
        widget.playerFactory?.call() ??
        (_chewieBackend != null
            ? Player(platformPlayer: _chewieBackend)
            : Player(
    ...
    _video = widget.videoBuilder == null
        ? VideoController(
            _chewieBackend?.legacyPlayer ?? _player,
  ...
  void didUpdateWidget(covariant PlayerScreen oldWidget) {
    ...
    if (oldWidget.playRequest == widget.playRequest) return;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted && !_closed) {
        unawaited(_play(widget.initialIndex, position: widget.initialPosition));
      }
    });
  }
  ...
  Future<void> _play(
    ...
    try {
      ...
      await _serialize(() async {
        ...
        final platform = _chewieBackend != null && plan.decryptionKey.isNotEmpty
            ? _chewieBackend!.legacyPlayer.platform
            : _player.platform;
        ...
        await _player.open(
          Media(
            plan.url,
            extras: {'decryptionKey': plan.decryptionKey},
        ...
        if (_closed || ticket != _generation) return;
        final mobileVideo = _chewieBackend?.videoController;
        if (mobileVideo != null) {
          _chewieController = ChewieController(
            videoPlayerController: mobileVideo,
            showControls: false,
  ...
  void _back() {
    if (_showFullscreen && !_television) {
      _rotate();
    } else if (widget.onInlineBack != null) {
      widget.onInlineBack!();
    } else {
  ...
  Widget _buildPlayer(BuildContext context) {
    ...
    return PopScope(
      canPop: _television || !fullscreen && widget.onInlineBack == null,
      ...
      child: CallbackShortcuts(
        ...
        child: Focus(
          ...
          child: Scaffold(
            backgroundColor:
                fullscreen || pictureInPicture || widget.mobileDetails != null
                ? Colors.black
                ...
                : SafeArea(
                    ...
                    child: LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        if (_mobile) {
                          final height = widget.mobileDetails == null
                              ? (constraints.maxWidth / _aspectRatio).clamp(
                                  0.0,
  ...
  Widget _videoPane(BuildContext context) {
    ...
    final videoTheme = _television
        ? televisionTheme(darkTheme)
        : _mobile
        ? darkTheme.copyWith(
            colorScheme: darkTheme.colorScheme.copyWith(
              primary: const Color(0xFFFB7299),
              primaryContainer: const Color(0xFF5E2940),
            ),
          )
        : darkTheme;
    final title =
            ...
            children: [
              if (widget.videoBuilder != null)
                widget.videoBuilder!(layeredControls)
                  ...
                  _chewieBackend?.usingLegacy == false)
                Stack(
                  fit: StackFit.expand,
                  children: [
