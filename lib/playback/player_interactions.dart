...
class PlayerInteractions extends ChangeNotifier {
  ...
  void pointerUp(PointerUpEvent event) {
    ...
    _origin = null;
    _endHold();
    if (verticalSwipe && available()) {
      hint(onEpisode(delta.dy < 0 ? 1 : -1));
    } else if (horizontalSwipe && available()) {
      final seconds = (delta.dx.abs() / _swipeWidth * 60).round().clamp(5, 60);
      seek(delta.dx > 0 ? seconds : -seconds);
    }
  }
