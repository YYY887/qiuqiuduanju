...
import '../library/local_media_screen.dart';
...
class _DownloadsScreenState extends State<DownloadsScreen> {
  ...
  Future<void> _refresh() async {
    if (_refreshing || !_allowed || !widget.active) return;
    _refreshing = true;
    try {
  ...
  void _selectAll(Iterable<DownloadJob> jobs) {
    setState(() {
      _selecting = true;
      _selected.addAll(jobs.map((job) => job.id));
    });
  }
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const Spacer(),
                TextButton(
                  onPressed: _busy || collections.isEmpty
                      ? null
                      : () => _selectAll(collections.expand((c) => c.jobs)),
