...
def build_core(simulator=False, variant=BuildVariant()):
    ...
    for sdk, architecture, triple in slices:
        ...
        run([go, 'build', '-buildvcs=false', '-trimpath', '-buildmode=c-archive', '-ldflags=' + variant.linker_flags,
...
def main():
    parser = argparse.ArgumentParser(description='构建秋秋短剧 iOS 核心和应用')
    ...
    if options.export_options:
        ...
        with tempfile.TemporaryDirectory(prefix='duanju-ios-') as temporary:
            payload = Path(temporary) / 'Payload'
            payload.mkdir()
            run(['ditto', str(application), str(payload / 'Runner.app')])
