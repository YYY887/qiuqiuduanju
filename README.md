...
手机底部导航使用 `liquid_glass_widgets` 的 `GlassTabBar.bottom`，提供液态玻璃折射、悬浮胶囊和选中态过渡；图标和文字常显，浅色 / 深色主题分别调整颜色。系统虚拟按键 / 手势区域延续当前背景，系统图标随主题调整明暗。导航支持系统大字设置、键盘焦点和无障碍选中状态。
...
**Release Android and iOS** 是独立的手动发布流程，只构建秋秋短剧全站源版（`--all-sources`）。先将本工作流并入仓库默认分支，推送与 `pubspec.yaml` 完全对应的标签（例如 `v0.2.70+76`），然后在 Actions 中手动运行并输入该标签。工作流固定使用该标签对应的提交，生成三种架构的已签名 APK、一个未签名 iOS IPA 和统一的 `SHA256SUMS.txt`；两端均成功后才将文件上传至当前仓库同名 GitHub Release。未签名 IPA 需自行签名后才能安装。若该 Release 已发布，工作流不会覆盖；若上传中断留下草稿，可重新运行以完成草稿。该工作流尚未执行发布验证。
...
尚未完成集中验证和平台验收时，当前版本 `0.2.70+76` 仅作为未验证开发快照，不标为完成验收的正式版本。源码压缩包由收尾脚本按 `真果·鉴-YYYYMMDDHHMM.zip` 生成。此前版本的历史记录仍保留在正式源码仓库中；Git 提交、标签和远程发布在用户明确要求时单独执行，源码收尾脚本不处理 Git。源码同步一致性确认仍执行。
