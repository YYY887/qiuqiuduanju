...
Flutter 多端独立短剧应用，原名“真果鉴 / 红果鉴”。站源请求、解析、下载和播放均在设备上完成，不依赖旧项目或自建服务。当前源码版本：**0.2.71+77（未验证开发快照）**。Android 与 iOS 应用图标使用用户提供的蓝色玩偶照片。手机底部为“首页 / 榜单 / 我的”三栏，悬浮导航居中且宽度受限；导航条自身采用浅色与深色主题均可辨认的液态玻璃底板，周围页面区域透明，列表内容延伸到导航后方，末尾可继续上滑避开导航。首页固定搜索与普通分类标签，分类文字与浅暖黄红渐变背景协调，站源和收藏、历史、下载入口统一放在“我的”；切换站源首次需要输入密码 `987654321`，验证成功后在本机记住解锁状态。搜索栏右侧的历史图标可直接打开观看记录；下载选集、批量下载和下载任务多选均提供全选与全不选。列表下滑自动加载下一页且不显示加载按钮；详情页点击继续播放或选集后在顶部原位播放。手机首页移除“今日推荐”大卡片，保留近期热播横向列表；左右拖动分类内容可跟手预览相邻页面，过半屏或快速滑动后切换并加载目标分类，顶部分类选中态同步变化，首次加载状态与滑入页保持一致。手机首页网格按可见区域构建；页面背景从浅暖黄红向下渐变。设备启动由用户自行运行验证，其余平台和功能仍按下文状态等待集中验证。Web / Chrome 不在当前交付范围。

按用户 2026-09-21 的要求，继续暂停整体验证。启动、榜单、画质增强、站源改名、画中画、连续播放控制栏、红果系列剧提醒、播放器 Tab 化、首页 / 播放页优化、多站源站源修复和启动登录开关均保留未验证快照状态；本轮完成界面源码修改及格式整理；GitHub Actions 已完成 Android 与 iOS 发布构建，未执行静态分析、自动化或设备验证。历史版本的检查记录不能作为当前源码的验收结论。

| 编译方式 | 应用名称 | 可用站源 |
| --- | --- | --- |
| 默认，不加参数 | 秋秋短剧 | 仅红果 |
| 构建脚本加 `--all-sources` | 秋秋短剧 | 红果、黄豆、剧果、野果、帝果、黄果视频、黄果 AI、黄果旧版 |
...
这是编译选项，应用内不能切换版本。两版显示名统一为“秋秋短剧”，分发文件名仍用原版别标识以区分站源范围。界面、站源调用、原生下载调度同时限制可用站源；红果版不会访问或继续执行其他站源的旧任务。
...
| 功能 | 操作 |
| --- | --- |
| 浏览 | 手机首页固定搜索栏和可横向滚动的普通分类标签，选中项使用下划线；下方显示近期热播横向剧集区和海报网格，内容区左右拖动可跟手预览相邻分类，过半屏或快速滑动后完成切换，再加载目标分类内容，顶部分类选中态同步移动。手机底栏为首页、榜单、我的悬浮玻璃导航；站源切换、排序、手动更新以及收藏、历史、下载从“我的”进入。支持默认、名称、自然季号、上线日期、热度、播放量排序及连载状态筛选；分类与榜单已加载内容在切回时复用。手机网格仅构建邻近可见区域，接近底部自动请求站源下一页，继续下滑也会触发续页，无需点击加载按钮 |
...
| 详情 | 手机点剧先进入详情页，页面依次显示播放预览、剧集信息与操作、播放源和选集；点继续播放或选集时直接在顶部原位播放，下方详情保持可见。30 分钟内重复进入优先使用详情缓存，手机选集默认展开。桌面和电视保留原有入口与资料、选集布局；选集每组 50 集，可切换范围、定位当前或输入集数跳转；红果同系列季显示为“相关推荐” |
...
| 手机播放 | 详情页顶部原位播放，视频下方保留详情、播放源和选集；其他独立播放入口仍使用选集 / 简介 / 下载 Tab。画面上滑下一集、下滑上一集，左右滑动后退 / 快进 5–60 秒；长按 350 毫秒临时 3 倍速，松开恢复。竖屏轻点播放 / 暂停，横屏轻点控制条，双击播放 / 暂停；返回、旋转、全屏、倍速、清晰度、推送、弹幕和画中画在播放器画面控制层；上一集 / 播放暂停 / 下一集只保留居中一组；进度条和菜单操作独立于画面手势 |
...
手机播放画面沿用原有切集、弹幕和菜单操作，控制层改为接近 B 站的深色渐变、粉色进度与选中态；隐藏控制层时显示细进度线。普通视频由 Chewie + video_player 播放，带解密密钥或特殊 URI 的视频继续使用原内核，以保留这些站源的播放能力。电视和 Windows 保持原播放器。
...
手机底部导航使用 `liquid_glass_widgets` 的 `GlassTabBar.bottom`，提供液态玻璃折射、悬浮胶囊和选中态过渡；选中胶囊与玻璃底板保持可见色差，导航贴近系统安全区上沿；图标和文字常显，浅色 / 深色主题分别调整颜色。系统虚拟按键 / 手势区域延续当前背景，系统图标随主题调整明暗。导航支持系统大字设置、键盘焦点和无障碍选中状态。
...
正常播放与下载**不转码、不降低清晰度**。Android / iOS 手机普通地址由 Chewie + video_player 调用系统播放后端；带解密密钥的地址、Windows 与电视继续使用 media_kit / libmpv。0.2.12 将远程 MP4 与 HLS 统一接入设备内流服务，处理请求头、密钥、预取缓存及应用代理，不经远程中转。本地已下载视频继续读取设备文件。
...
| 已核实事实 | 对实施的要求 |
| --- | --- |
| 项目使用 `media_kit 1.2.6`、`media_kit_video 2.0.1`；原播放链路直接创建 `VideoController` | 0.2.18 固定这两个版本，并从 `lib/playback/player_screen.dart` 接入统一增强控制器、偏好与菜单 |
...
| 模块 | 责任与边界 |
| --- | --- |
| `VideoEnhancementController`，从 `lib/playback/player_screen.dart` 接入 | 负责选档、输入变化、状态、错误恢复和释放；复用当前播放生命周期，操作携带播放器实例与分集代次，丢弃过期结果 |
...
| `lib/playback/player_menu.dart`、`lib/playback/television_controls.dart` | 共用选项和当前状态；手机、桌面及 TV 均有可发现入口，不依赖手势才能操作 |
...
当前集中验证暂停，**Build app packages** 只保留手动触发；恢复验证后运行它会先检查再构建：
...
| `hongguojian-ios-unsigned` | `zhenguojian-ios-unsigned` | 未签名 IPA 和 SHA256，需自行签名后才能安装 |

Actions 分别传入默认参数与 `--all-sources` 构建两版，Flutter 和 Go 回归也覆盖两种编译配置。产物保留 14 天，不自动创建 GitHub Release。首次平台构建结果以实际 Actions 输出为准。

**Release Android and iOS** 是独立的手动发布流程，只构建秋秋短剧全站源版（`--all-sources`）。先将本工作流并入仓库默认分支，推送与 `pubspec.yaml` 完全对应的标签（例如 `v0.2.71+77`），然后在 Actions 中手动运行并输入该标签。工作流固定使用该标签对应的提交，生成三种架构的已签名 APK、一个未签名 iOS IPA 和统一的 `SHA256SUMS.txt`；两端均成功后才将文件上传至当前仓库同名 GitHub Release。未签名 IPA 需自行签名后才能安装。若该 Release 已发布，工作流不会覆盖；若上传中断留下草稿，可重新运行以完成草稿。0.2.71+77 的发布构建已通过，三架构 APK、无签名 IPA 和 SHA256SUMS.txt 已上传至 GitHub Release；设备行为由用户自行验证。
...
未配置时生成预览 APK，不同构建机的预览签名可能无法相互覆盖。手动 Release 流程要求上述四个 Android Secrets 完整配置，缺少时直接失败，不会把预览签名当作正式安装包发布。iOS 未签名产物不需要 Apple 证书、描述文件或对应 Secrets。创建 Android 签名文件并保存到项目外：
...
`lib/main.dart` 是应用入口。其余 Dart 源码按职责放在以下目录；测试和平台工程仍使用根目录的 `test/`、`integration_test/`、`android/`、`windows/` 与 `ios/`。

| 目录 | 内容 |
| --- | --- |
| `lib/app/` | 编译配置、平台布局、主题、方向与应用导航 |
| `lib/core/` | 公共模型与 Go 原生核心桥接 |
| `lib/catalog/` | 站源、目录、搜索、推荐与榜单 |
| `lib/ui/` | 首页、详情与通用界面组件 |
| `lib/playback/` | 播放器、播放恢复、遥控与画质增强 |
| `lib/downloads/` | 下载任务、选集、偏好与后台下载 |
| `lib/library/` | 本地记录、追剧、媒体库、合并与导出 |
| `lib/lan/` | 局域网发现、同步与推送播放 |
| `lib/danmaku/` | 弹幕模型、控制与绘制 |
| `lib/settings/` | 用户、资源与应用设置界面 |

原生业务核心位于 `native/core/`，FFI 导出位于 `native/bridge/`；`scripts/` 放构建与源码收尾脚本。当前项目没有 Web 工程，原生 FFI 与播放器也未适配浏览器。
...
尚未完成集中验证和平台验收时，当前版本 `0.2.71+77` 仅作为未验证开发快照，不标为完成验收的正式版本。源码压缩包由收尾脚本按 `真果·鉴-YYYYMMDDHHMM.zip` 生成。此前版本的历史记录仍保留在正式源码仓库中；Git 提交、标签和远程发布在用户明确要求时单独执行，源码收尾脚本不处理 Git。源码同步一致性确认仍执行。
...
0.2.71+77 将手机悬浮底部导航下移 16dp：外层去掉额外 8dp 底部间距，玻璃组件上下内边距从 12dp 收到 4dp；保留系统手势安全区、玻璃底板和选中项暖色对比。已整理源码格式；Android 三架构签名 APK 与 iOS 无签名 IPA 在 [v0.2.71+77 发布流程](https://github.com/YYY887/qiuqiuduanju/actions/runs/36012872235) 中构建并通过产物校验，已上传到 [GitHub Release](https://github.com/YYY887/qiuqiuduanju/releases/tag/v0.2.71%2B77)；界面表现待用户设备验证。

0.2.70+76 Android 与 iOS 应用图标改用用户提供的蓝色玩偶照片，原图保存在 `assets/branding/app_icon.jpg`。Android 入口改用生成的 `mipmap/ic_launcher`，iOS 的 `AppIcon.appiconset` 已同步生成；后续修改原图可运行 `dart run flutter_launcher_icons` 重新生成。手机底部导航选中项采用与整条玻璃底板有明显色差的暖色胶囊。手动发布流程改为上传三架构签名 APK 与未签名 IPA，常规检查与构建工作流在集中验证恢复前只手动触发。源码格式已整理；发布结果以 Actions 为准，设备行为仍待用户验证。

0.2.69+75 切换站源首次需要输入密码 `987654321`，验证成功后在本机保存解锁状态，后续切换无需重复输入；取消或密码错误时保持当前站源。手机首页搜索栏右侧新增观看历史入口，复用现有最近观看页面；未改变“我的”页面构造函数，避免热重载时旧实例与新增必填参数冲突。下载选集、批量下载和下载任务多选提供明确的全选与全不选操作。主题选择与站源解锁均在弹窗退出动画结束后写入本机配置，避免弹窗退出期间重建应用主题。手机悬浮导航条设置可见的深浅色玻璃底板，导航周围仍透明。仅完成源码和格式整理，未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.68+74 手机底部悬浮导航恢复导航条自身的液态玻璃背景，保留选中项的半透明玻璃效果；导航周围区域继续透明，首页内容仍可延伸到导航后方并在列表末尾上滑避开导航。仅完成源码和格式整理，未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.67+73 手机首页分类内容支持跟手横向拖动：当前页与相邻分类页同步移动，拖过半屏或快速滑动后完成切换，顶部分类选中态随拖动阈值更新。拖动位移、相邻页预览和分类高亮分别局部更新，不在手指移动期间重建整个首页；目标分类有快照时预览热播区和网格，首次进入时预览与正式页面使用相同加载状态，滑动完成后再请求内容，减少切换闪动。首页不再整页预留底栏高度，内容从透明悬浮导航后方延伸，列表末尾单独留出可滚动空间；导航居中限制宽度并避让系统手势区，选中项采用半透明玻璃。仅完成源码和格式整理，未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.66+72 悬浮底栏保留液态玻璃选中项，将整条导航轨道改为无填色、无模糊的透明背景，让页面渐变直接透出。仅完成源码和格式整理，未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.65+71 手机底部恢复悬浮液态玻璃导航，选中图标与分类文字统一采用暖红主色，未选分类文字改为适合暖色渐变背景的深棕或浅暖色。手机首页去除 36 部一批的界面扩容，使用懒构建网格和站源分页；邻近行提前构建、海报按显示宽度解码，并隔离静态渐变背景绘制，以减少上下滚动时的重建和绘制开销。仅完成源码和格式整理，未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.64+70 手机首页内容区为悬浮底栏预留空间，避免底部加载提示被玻璃导航遮住；底栏背景透明，选中项保留液态玻璃效果。首页和榜单移除手机端“加载更多”按钮，滚动接近底部或继续下滑时自动续页。详情页点击“继续播放”或选集后复用既有播放器，在当前页顶部直接播放，下方仍为资料和选集；关闭播放恢复封面预览，全屏继续使用播放器原有控制。分类动画控制器改为按需创建，避免热重载保留旧首页状态时出现 `LateInitializationError`。仅完成源码和格式整理，未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.63+69 移除手机首页“今日推荐”大卡片，近期热播列表从首部剧开始展示。左右滑动切分类支持滑动距离或速度触发，新内容使用轻量位移过渡；动画只变换内容位置，保留同一滚动组件与已缓存的分类数据。仅完成源码和格式整理，未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.62+68 软件显示名统一改为“秋秋短剧”，重生成电视横幅文字；手机底栏改为“首页 / 榜单 / 我的”，保留原 `GlassTabBar` 液态玻璃组件并让栏外区域透出内容。“我的”集中站源切换、收藏追剧、历史、下载与设置；首页搜索和普通分类标签固定在内容上方。手机首页、榜单与“我的”首次打开后保留页面状态，分类与榜单切回时使用内存快照，剧集详情重复进入时优先使用 30 分钟内的缓存；切页使用轻量位移动画，下载页隐藏时暂停定时刷新。详情页参照用户截图调整为播放预览、信息操作、播放源和选集的连续布局；手机背景从浅暖黄红向下渐变。仅完成源码和格式整理，未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.61+67 手机发现页的站源操作栏、搜索栏与分类栏改为随列表滚动，不再固定在海报网格上方；切换分类时保持同一个滚动区域与分类控件，复用未变化的筛选排序结果，降低液态玻璃滑块的独立图层和形变开销，并移除首页内容外层的重复绘制边界，以缓解切换卡顿。仅完成源码和格式整理，未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.60+66 按用户界面截图重排手机发现页：搜索栏常显，加入数据驱动的绿色推荐主视觉、横向剧集区和下方海报网格，手机底栏改为绿色选中态。首页、推荐和榜单共用的分类栏接入 `GlassSegmentedControl.scrollable`：选中块按液态玻璃动画滑动，选中块可拖动切类，其余区域可横向滚动；电视分类操作维持原方式。手机主页面移除整页透明度和位移动画，保留底栏自身选中动画，并跳过追剧与历史空搜索时逐条规范化文本的工作，以减轻切页卡顿。仅完成源码和格式整理，未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.58+64 修正手机横屏导航：之前仅以视口宽度 840 像素为界，横屏手机会进入桌面 `NavigationRail` 布局；现在只有 Windows、macOS、Linux 的宽屏窗口使用左侧导航，Android / iOS 横屏沿用底部液态玻璃导航，并将横屏底栏限制在 480 像素宽、居中显示。仅完成源码和格式整理，未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.57+63 手机播放器增加左右滑动进度跳转和底部全屏按钮；发现页每批向网格提供 36 部，滚动接近末尾先展示下一批，再按需请求站源下一页；手机主导航切页增加淡入位移过渡。用户日志中的 `liquid_glass_widgets` 找不到是旧 Flutter 运行会话热重载时未重新解析新增依赖；本地 `pubspec.lock` 和 `.dart_tool/package_config.json` 已包含该包，需结束当前运行后重新 `flutter pub get --enforce-lockfile`、`flutter run`。未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.56+62 手机底部导航改用 `liquid_glass_widgets` 的 `GlassTabBar.bottom`，替代上一版手写模糊栏；Android / iOS 手机普通视频接入 Chewie + video_player，保留原控制层的切集、弹幕、断点和全屏操作并增加粉色进度与隐藏时细进度线。带解密密钥的流继续由原内核播放；Windows 与电视不改。仅完成源码和依赖接入、格式整理，未执行静态分析、构建、自动化或设备验证，仍为未验证开发快照。

0.2.55+61 参考 `qianqiupay_flutter` 的玻璃底栏视觉，为手机导航增加模糊玻璃、浅深色半透明表面、悬浮边距及选中胶囊；保留原有导航语义和桌面 / 电视布局。手动 GitHub Release 的 iOS 构建改为未签名 `.app` ZIP，移除证书、描述文件和 IPA 导出要求，Android 签名要求保持。仅整理源码格式，未运行 Actions、打包、静态分析或设备验证，仍为未验证开发快照。

0.2.54+60 新增手动 GitHub Release 工作流：按版本标签构建真果鉴三架构签名 APK 与 Ad Hoc 签名 IPA，两端产物及校验值齐全后再发布到当前仓库；补充 iOS 临时签名配置，避免签名材料进入源码。仅核对源码语法和配置关联，未执行 Actions、打包、自动化或设备验证，仍为未验证开发快照。

0.2.53+59 用户直接执行 `flutter run` 后出现“本地核心加载失败”。核对设备 ABI 为 ARM64，原 Debug APK 没有 `libduanju_core.so`，项目 `android/app/src/main/jniLibs` 也没有预编译核心；直接运行 Flutter 不会自动执行 Go 构建脚本。按开发步骤编译 ARM64 核心时，Go 在没有 `.git` 的纯源码目录因 VCS stamping 失败，现为 Android、Windows 与 iOS 的 Go 构建显式设置 `-buildvcs=false`。已成功生成 ARM64 原生核心；用户明确要“真果鉴”后，当前本机核心已重新编译为全站源版，运行时需传 `--dart-define=ALL_SOURCES=true` 才与核心一致。按用户要求不单独打包或代为运行设备，启动由用户验证。其余测试与回归仍暂停。

0.2.52+58 沿用现有珊瑚色主题，调整浅色与深色表面层级，统一按钮、输入框和分类标签；发现页增加内容标题与已加载数量，海报卡片增加轻量阴影、集数标签和更清晰的标题层级；底部导航改为选中胶囊，空状态使用统一图标容器。同步更新桌面端关于页使用的版本常量。已完成修改文件的 Dart 格式整理；按当前约定未执行静态分析、构建、自动化或设备回归，视觉效果仍为未验证开发快照。

0.2.51+57 将 `lib/` 下 80 个 Dart 文件按职责归档，更新源码、测试和集成测试的导入路径；保留 `lib/main.dart` 作为入口。已完成 Dart 格式整理及本地导入路径存在性核对；按当前约定未执行静态分析、构建、自动化或设备回归，仍为未验证开发快照。

0.2.50+56 新增管理员“启动时需要登录”开关：设置管理员密码后仍默认保持启动登录，管理员可在用户管理中关闭；关闭后保留密码保护，只在切换用户或手动锁定时验证。此前核对剧果、野果、帝果的目录分页、榜单和播放解析链路：多站源更新默认批量 50 页，野果目录 / 搜索 / 播放使用 POST，剧果保留 CloudFront 签名 Cookie 到播放列表、分片、预加载和下载，帝果 vplayer 签名失败不再静默回退到未签名地址。野果运行时固定优先使用当前线路 `https://analyze.buxefaex.cc/`，通过 `https://ygdj7.com/` 发现新线路；旧域名仅保留缓存与身份兼容识别，不再主动作为接口回退地址；首页和推荐卡片点击后在当前页读取详情，成功后直接进入播放器，避免“正在进入播放”的中间页闪现；站源管理列表底部避让系统虚拟导航栏，避免最后一组站源被遮挡；播放页轻量 Tab 与紧凑选集网格之间补充少量间距，避免按钮贴得过紧；修复野果线路发现正则在原生核心初始化时触发 Go panic 导致 Android 启动闪退的问题。该轮只维护源码和定向测试，不打包 APK、不安装设备、不做真实播放验收。
...
0.2.39+45 继续按播放页 UI/UX 反馈优化：播放页下方 Tab 改为“选集 / 简介 / 下载”，原“相关”不再放在播放页；红果同系列相关推荐仍保留在详情页，避免播放页出现经常为空的内容区。追剧入口从播放器覆盖层移到简介内容区；下载入口从播放器覆盖层移到播放页下载 Tab，下载选择器支持嵌入式紧凑布局并复用连续选集网格。播放器覆盖层移除移动端音量、下载和追剧按钮，桌面端保留音量，电视端不再显示推送按钮；弹幕按钮改为圆形“弹”字样。首页目录滚动接近底部自动加载下一页，底部“加载更多”保留为兜底入口。已执行 `dart analyze lib/ui/home_screen.dart lib/playback/player_controls.dart lib/playback/player_screen.dart lib/downloads/download_picker.dart test/player_screen_test.dart test/downloads_test.dart test/danmaku_widget_test.dart`，仅剩既有 info 级 lint；待执行播放器、下载、首页分页定向测试和 Android Debug 构建。未生成 Release APK，真实设备视觉验收仍待集中执行。

0.2.38+44 继续修正播放器实机截图反馈：非全屏点击画中画后，整个播放页在 PiP requested / active 状态下只渲染视频 Pane，不再把选集 / 简介 / 相关 Tab 内容缩进系统小窗；`sourceRectHint` 改为在纯视频布局重绘后一帧再计算，和实际进入小窗的画面一致。全屏播放器顶部显式使用 `viewPadding.top + 16` 且至少 40dp 避让，避免返回 / 标题与状态栏网络、电量、时间重叠；底部显式使用 `viewPadding.bottom + 24` 且至少 64dp，把进度条和底部工具整体抬高，避开虚拟导航栏；PiP 请求和激活期间连保存进度告警也不再绘制。已执行 `dart analyze lib/playback/player_controls.dart lib/playback/player_screen.dart test/player_screen_test.dart`，仅剩既有 info 级 lint；已执行 `flutter test --no-pub --dart-define=DISABLE_REMOTE_IMAGES=true test/player_screen_test.dart`、`flutter test --no-pub --dart-define=DISABLE_REMOTE_IMAGES=true test/danmaku_widget_test.dart`；已执行 `./gradlew :app:assembleDebug` 成功。未生成 Release APK，真实 Android 播放小窗和全屏控件截图仍待集中设备验收。

0.2.37+43 优化 Android 画中画小窗：参考 Android PiP 和 Apple 小窗播放最佳实践，进入画中画后 Flutter 自绘播放器控件、弹幕覆盖、进度条、标题、加载和错误覆盖层均不再绘制，小窗只保留视频画面，播放 / 暂停 / 返回应用 / 关闭等交互交给系统 PiP 控件；进入前先隐藏覆盖层并等待一帧，再调用原生 `enterPictureInPictureMode`，同时传入视频区域 `sourceRectHint`，避免系统截取到带按钮的旧界面并优化转场动画。已新增 `test/player_screen_test.dart` 用例覆盖“画中画隐藏 App 覆盖控制层”。已执行 `dart analyze lib/playback/player_controls.dart lib/playback/player_screen.dart test/player_screen_test.dart`，仅剩既有 info 级 lint；已执行 `flutter test --no-pub --dart-define=DISABLE_REMOTE_IMAGES=true test/player_screen_test.dart`、`flutter test --no-pub --dart-define=DISABLE_REMOTE_IMAGES=true test/danmaku_widget_test.dart`；已执行 `./gradlew :app:assembleDebug` 成功生成 162 MB Debug APK，并通过安装辅助脚本安装到 Android 15 / API 35 真机 V2453A，启动到首页且 logcat 未见 App 崩溃。自动点播进入真实 PiP 时设备会话被 vivo 系统 single-cleaner 强停，未完成真实小窗截图验收；Release APK 也未在本轮生成。

0.2.36+42 微调播放器布局：移除底部中间工具区的横向滚动容器，保留左侧倍速 / 清晰度、中间推送 / 下载 / 追剧、右侧音量 / 画中画 / 全屏或选集的稳定分布；红果弹幕按钮移到播放器右侧中段独立悬浮，避开中央播放按钮和底部进度条，弹幕开关和失败重试回调保持不变。已执行 `dart analyze lib/playback/player_controls.dart`、`flutter test --no-pub --dart-define=DISABLE_REMOTE_IMAGES=true test/player_screen_test.dart`、`flutter test --no-pub --dart-define=DISABLE_REMOTE_IMAGES=true test/danmaku_widget_test.dart`。本轮未打包 APK、未安装设备，真实站源和平台行为仍待集中验证。

0.2.35+41 修正播放器按钮布局继续反馈：按 Material / Android 触控目标规范和 Apple 自定义视频播放控件建议，把倍速、清晰度、推送、下载、追剧、弹幕、音量、画中画等入口统一为 36 高视觉工具按钮，外层保留更大的可点区域；移动和桌面底部控制层改为左侧倍速 / 清晰度、中间内容动作、右侧音量 / 画中画 / 全屏或选集分组，宽度不足时中间动作区可横向容纳，避免所有按钮堆到中间。音量不再打开居中大弹窗，改为右下角小型竖向滑动面板，支持拖动和一键静音 / 恢复。已执行 `dart analyze lib/playback/player_controls.dart`、`flutter test --no-pub --dart-define=DISABLE_REMOTE_IMAGES=true test/player_screen_test.dart`、`flutter test --no-pub --dart-define=DISABLE_REMOTE_IMAGES=true test/danmaku_widget_test.dart`。`flutter analyze lib/playback/player_controls.dart lib/playback/player_screen.dart lib/lan/lan_controller.dart` 在中文项目路径下再次触发 analysis server LSP JSON 解析异常，未得到有效报告；本轮未打包 APK、未安装设备，真实站源和平台行为仍待集中验证。

0.2.34+40 修复移动播放器继续反馈：选集 Tab 的连续网格顶部恢复区间选择器，可快速定位每 50 集开头；播放器底部改为工具按钮一行、时间与进度条同一行，移动端去掉底部重复旋转按钮，顶部返回 / 旋转按钮增加安全间距，音量按钮在手机播放器始终显示；局域网投屏按钮尺寸与其他播放器工具按钮统一。红果相关推荐新增本地剧库系列候选缓存，详情和播放页相关不再只依赖追剧 / 历史 / 新季提醒，支持聚宝仙盆这类多季剧展示同系列。已执行 `dart analyze lib/playback/player_controls.dart lib/playback/episode_browser.dart lib/library/local_store.dart lib/library/local_snapshot.dart lib/playback/player_screen.dart lib/ui/detail_screen.dart lib/lan/lan_screen.dart test/follow_state_test.dart test/library_features_widget_test.dart test/player_screen_test.dart test/danmaku_widget_test.dart`，仅剩既有 info 级 lint；已执行 `flutter test --no-pub --dart-define=DISABLE_REMOTE_IMAGES=true test/follow_state_test.dart`、`test/library_features_widget_test.dart --plain-name "detail related series can use refreshed catalog candidates"`、`test/player_screen_test.dart`、`test/danmaku_widget_test.dart`、`test/downloads_test.dart --plain-name "detail download picker handles VIP, quality and added=2"`。本轮未打包 APK、未安装设备，真实站源和平台行为仍待集中验证。

0.2.33+39 继续优化播放器控制层与移动播放页：手机和桌面播放器覆盖层移除“播放设置”按钮，倍速、清晰度、推送、下载、追剧、弹幕、音量、画中画和旋转直接可用；自动连播和下一集预加载默认始终开启；上一集 / 播放暂停 / 下一集只保留居中一组，非全屏不再在播放器层重复显示选集按钮。移动端 Tab 增大点击区域，选集改为带边框的连续滚动紧凑网格，简介保留封面，相关列表显示红果同系列封面；选集按钮区分焦点和当前集，避免看起来多集同时激活。播放器缓冲进度改用进度条二级轨道，不再显示“已缓冲至”文字；下载选集页提供“取消全选”。已执行 `dart analyze lib/playback/player_controls.dart lib/playback/player_screen.dart lib/playback/episode_browser.dart lib/ui/remote_widgets.dart lib/downloads/download_picker.dart lib/playback/playback_buffer.dart test/danmaku_widget_test.dart test/player_screen_test.dart test/downloads_test.dart`，仅剩既有 info 级 lint；已执行 `flutter test --no-pub --dart-define=DISABLE_REMOTE_IMAGES=true test/player_screen_test.dart`、`test/danmaku_widget_test.dart`、`test/downloads_test.dart --plain-name "detail download picker handles VIP, quality and added=2"` 和 `added=0`。本轮未打包 APK、未安装设备，真实站源和平台行为仍待集中验证。

0.2.32+38 优化播放入口与移动播放页：普通点剧先读取详情后直接进入播放器，下载入口仍走详情页；手机播放器下方用“选集 / 简介 / 相关”Tab 承接原详情内容，红果同系列剧在相关 Tab 中打开；手机播放页移除额外顶部标题栏，返回和旋转放入视频控制层，倍速、清晰度、推送、上一集、下一集、选集和设置不再在视频下方重复出现；移动端选集改为 30 集一组紧凑方格，分组、当前集和跳转合并在同一行；首页移除分类下方的更新状态提示，保留刷新任务；详情页选集默认折叠且只高亮当前播放集；下载选集页显式提供“全选 / 清空 / 仅非 VIP”与已选数量。已执行 `dart analyze lib/playback/player_screen.dart lib/playback/player_controls.dart lib/playback/episode_browser.dart lib/ui/remote_widgets.dart lib/playback/player_menu.dart test/player_screen_test.dart test/theme_test.dart`，仅剩既有 info 级 lint；已执行 `flutter test --no-pub --dart-define=DISABLE_REMOTE_IMAGES=true test/player_screen_test.dart` 和 `test/theme_test.dart --plain-name "player shell follows light theme while video controls stay dark"`。此前本轮还执行过 `test/library_features_widget.dart` 和下载选择定向测试。未打包 APK、未安装设备，真实站源和平台行为仍待集中验证；完整 `test/downloads_test.dart` 仍有既有下载合集默认折叠导致的旧断言失败，未作为本轮通过项。
...
0.2.21+27 修正播放器页面外壳跟随浅色 / 深色主题，视频画布与覆盖控制保持深色；画质增强关闭时隐藏画面类型和原画对比，开启后展开配置；原画对比不再等待渲染确认成功才可用；Android 自动档先保流畅使用高质量缩放，清晰优先才尝试更高负载增强。本轮通过 ADB 确认当前前台 App 为 `com.duanju.duanju_app/.MainActivity`、进程 `29688`，并按 `logcat --pid=29688` 定位当前 App 日志；当时该进程没有可读历史日志行。清理本项目可再生成的 `build` / `.dart_tool` 缓存后，窄范围 `dart analyze lib/playback/player_screen.dart lib/playback/player_menu.dart lib/playback/video_enhancement_settings.dart lib/playback/video_enhancement.dart lib/playback/video_enhancement_policy.dart test/theme_test.dart` 完成，仅剩既有 info 级 lint；新增播放器主题用例 `flutter test test/theme_test.dart --plain-name "player shell follows light theme while video controls stay dark"` 通过。整份 `test/theme_test.dart` 仍有既有导航断言 `settings changes open routes without losing the active tab` 未通过，未作为本轮验收结论；本轮没有平台构建、设备复验、新 APK、Windows 完整包或 iOS 应用。30 项已实现待验证仍保持未勾选。
