...
按用户 2026-09-21 的要求，继续暂停整体验证。启动、榜单、画质增强、站源改名、画中画、连续播放控制栏、红果系列剧提醒、播放器 Tab 化、首页 / 播放页优化、多站源站源修复和启动登录开关均保留未验证快照状态；本轮完成界面源码修改及格式整理，未执行静态分析、构建、自动化或设备验证。历史版本的检查记录不能作为当前源码的验收结论。
...
**Release Android and iOS** 是独立的手动发布流程，只构建秋秋短剧全站源版（`--all-sources`）。先将本工作流并入仓库默认分支，推送与 `pubspec.yaml` 完全对应的标签（例如 `v0.2.71+77`），然后在 Actions 中手动运行并输入该标签。工作流固定使用该标签对应的提交，生成三种架构的已签名 APK、一个未签名 iOS IPA 和统一的 `SHA256SUMS.txt`；两端均成功后才将文件上传至当前仓库同名 GitHub Release。未签名 IPA 需自行签名后才能安装。若该 Release 已发布，工作流不会覆盖；若上传中断留下草稿，可重新运行以完成草稿。发布构建与设备验证状态以本次 Actions 和用户设备运行结果为准。
...
0.2.71+77 将手机悬浮底部导航下移 16dp：外层去掉额外 8dp 底部间距，玻璃组件上下内边距从 12dp 收到 4dp；保留系统手势安全区、玻璃底板和选中项暖色对比。已整理源码格式；界面表现待用户设备验证。
